...
var image =[];
class UpdateSalientComponent extends React.Component{
    constructor(props){
        super(props)
        this.state={
            productId:this.props.match.params.idpro,
    ...
    saveCategory = (e) =>{
        ...
        SalientService.updateSalient(salient,this.state.id,this.state.productId).then(async (res) => {
            this.addProductImage(image);
            alert("Cập nhật tính năng thành công")
            this.props.history.goBack();
        })
    }
    componentDidMount(){
        SalientService.getSalientById(this.state.id).then((response) =>{
            let salient = response.data;
            this.setState({title: salient.title, images: salient.images, description:salient.description});
        })
    }
    ...
    render (){
        return (
         ...
         <BodyComponent/>
            ...
            />
            <img src={"http://localhost:8080/images/"+this.state.images} style={{width:"50px",height:"50px"}}/>
                </div>
                <div className="form-group">
                    <label >Mô tả</label>
                    <CKEditor
...
export default UpdateSalientComponent